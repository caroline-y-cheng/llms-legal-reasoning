...
train_test_dir = os.path.join(interp_dir, 'train_test_splits')
output_path = os.path.join(interp_dir, 'gpt4_1', 'gpt4_1_description_ft_results')
...
# train_path = 'train_test_splits/gpt_description_train_4.jsonl'
# eval_path = 'train_test_splits/gpt_description_test_4.jsonl'
...
# training_file_id='file-7Bg9a8V5CiuAEWDhVcQYA2'
# validation_file_id='file-DHSLZ68hYgbBw7uRog5e5b'

# openai.fine_tuning.jobs.create(
#   training_file=training_file_id,
#   validation_file=validation_file_id,
#   model="gpt-4.1-2025-04-14",
#   suffix='legal_reasoning_description'
# )
...
def get_descriptive_interp(text, formal_description, grand_description, none_description):
  ...
  response = openai.chat.completions.create(
      model="ft:gpt-4.1-2025-04-14:cornell-law-projects:legal-reasoning-description:Ce3PXMDE",
      max_tokens=3,
...
for split in range(4, 5): # limit to first five splits
...
for split in range(4, 5):
  ...
  print(y_test, clean_predicted_labels)
  class_report = classification_report(y_test, clean_predicted_labels, labels=np.unique(y_test), output_dict=True)
...
full_df.to_csv(os.path.join(output_path, 'gpt_description_ft_results.csv'))
