...
results_dir = os.path.join(formalism_dir, 'results')
if not os.path.exists(results_dir):
   os.makedirs(results_dir)

output_path = os.path.join(results_dir, 'multi-class')
...
models = ["legal-bert", "legal-roberta", "legal-modern-bert"] 
epoch_num = 3
...
for model_type in models:
  ...
  for split in range(0, 5):
    ...
    elif model_type == "legal-roberta":
      model_name = "saibo/legal-roberta-base"
      tokenizer = AutoTokenizer.from_pretrained(model_name)
      model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=len(id2label)).to(device_name)
    elif model_type == "legal-modern-bert":
      model_name = "Free-Law-Project/modernbert-embed-base_finetune_512"
      tokenizer = AutoTokenizer.from_pretrained(model_name)
      model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=len(id2label)).to(device_name)
    else:
    ...
    trainer.train()  
    model_save_path = os.path.join(output_path, f"{model_type}_split_{split}_model")
