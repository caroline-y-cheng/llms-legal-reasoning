...
for model_name, model in models.items():
  ...
  output_path = os.path.join(interp_dir, model_name, model_name + '_few_shot_results')
  ...
  def get_few_shot_interp(text, formal_example, grand_example, none_example):
      ...
      response = openai.chat.completions.create(
          model=model,
          max_tokens=3, # for gpt-4.1-2025-04-14
  ...
  for split in range(0, 5):
    ...
    class_report = classification_report(y_test, clean_predicted_labels, output_dict=True)
  ...
  full_df.to_csv(os.path.join(output_path, 'gpt_few_shot_results.csv'))
