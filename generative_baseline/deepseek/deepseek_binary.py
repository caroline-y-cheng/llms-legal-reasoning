...
for split in range(0, 5):
  ...
  with open(os.path.join(generated_output_path, f'predictions_{split}.txt'), 'r') as file:
    print(file)
    predicted_labels = [line.rstrip() for line in file if ("interpret" in line or "no" in line)]
...
full_df.to_csv(os.path.join(output_path, 'deepseek_interpretation_results.csv'))
