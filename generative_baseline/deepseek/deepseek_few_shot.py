...
client = Together(
    api_key=open(os.path.join(interp_dir, 'private', 'togetherai_api.txt')).read().strip(),
)
...
def get_few_shot_interp(text, formal_example, grand_example, none_example):
    ...
    full_prompt = f"{prompt}\n\"{text}\"\n\nYou must respond in a single word. Your options are any of \"GRAND\", \"FORMAL\", or \"NONE\". What is the one word that describes this paragraph?"
    response = client.chat.completions.create(
      model="deepseek-ai/DeepSeek-V3.1",
      max_tokens=5,
      messages = [{'role': 'user', 'content': full_prompt + text}],
      temperature=0.0,
  )
...
for split in range(0, 5):
  ...
  class_report = classification_report(y_test, predicted_labels, labels=np.unique(y_test), output_dict=True)
...
full_df.to_csv(os.path.join(output_path, 'deepseek_examples.csv'))
